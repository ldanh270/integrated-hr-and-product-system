...
export class CapacityDemoSeeder implements ISeeder {
  ...
  async run(context: SeedContext): Promise<Partial<SeedContext>> {
    ...
    if (!context.employees) {
      console.warn("CapacityCopilot: employees not found in context. Skipping demo seed.")
      return {}
    }
    const demoTester = context.employees.find((e) => e.username === "part_time")
    if (!demoTester) {
    ...
    console.log(
      "Demo tip: increase Deal target % to 80-100 to show shortage, lower it to 10-20 to show surplus.",
    )
