...
import { ACTION_TYPES_LIST, MOCK_IP_ADDRESSES } from "../data/auth.data.ts"
...
export const seedAuth = async (passedEmployees?: any[]): Promise<void> => {
  ...
  const hrManager =
    employees.find((e) => e.role === ROLE.HR_MANAGER || e.role === ROLE.ADMIN) || employees[0]
  ...
  for (let i = 0; i < resetEmployees.length; i++) {
    ...
    const status =
      i === 0 ? ("pending" as const) : i === 1 ? ("approved" as const) : ("rejected" as const)
    ...
    await PasswordResetRequest.create({
      ...
      note:
        status === "rejected"
          ? "Invalid request profile validation"
          : status === "approved"
            ? "Approved by HR"
            : null,
