...
// ─── WORKING SHIFT ───────────────────────────────────────────
export const createWorkingShiftSchema = z.object({
...
// ─── SHIFT SCHEDULE ──────────────────────────────────────────
export const assignShiftScheduleSchema = z.object({
  ...
  validTo: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date format" }).nullable().optional(),
}).strict()

export type AssignShiftScheduleSchemaType = z.infer<typeof assignShiftScheduleSchema>

// ─── EMPLOYEE SHIFT OVERRIDE ─────────────────────────────────
export const overrideEmployeeShiftSchema = z.object({
  employeeId: z.string().regex(objectIdRegex, "Invalid ObjectId"),
  shiftId: z.string().regex(objectIdRegex, "Invalid ObjectId"),
  assignedDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date format" }),
}).strict()
