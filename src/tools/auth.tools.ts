		...
		async ({ loginId }) => {
			try {
				...
				if (req.status === "pending") {
					return buildSuccess({
						status: "pending",
						message: "User has not completed login yet. Please wait and check again in a few seconds.",
					});
				}
				...
				return buildSuccess({
					...
					instruction: "CRITICAL: Keep this sessionId and pass it as an argument to all other tools.",
				});
			} catch (error: any) {
