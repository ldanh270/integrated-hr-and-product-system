...
This guide documents all available skills in the all project. Skills extend Agents's capabilities with specialized knowledge, workflows, and tool integrations.
...
Skills are specialized knowledge modules that enhance Agents's capabilities in specific domains. Each skill provides:
...
Skills are invoked automatically when Agents detects relevant context in your request. You can also explicitly invoke a skill:

```bash
# Automatic invocation
agents "How do I implement OAuth with Better Auth?"
...
# Edit .agents/.env and add your key

# 4. Shared skills configuration
cp .agents/skills/.env.example .agents/skills/.env
# Edit .agents/skills/.env and add your key

# 5. Individual skill directory
cp .agents/skills/gemini-audio/.env.example .agents/skills/gemini-audio/.env
...
- "Integrate external service with Agents"
...
```bash
agents "Create a skill for [technology/framework]"
```
...
### For Users
- Let Agents auto-detect which skill to use based on context
...
- **List skills:** `ls .agents/skills/`
- **View skill:** `cat .agents/skills/[skill-name]/SKILL.md`
- **Get help:** Ask Agents "How do I use the [skill-name] skill?"
