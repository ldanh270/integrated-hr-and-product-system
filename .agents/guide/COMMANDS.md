...
A comprehensive guide to all available slash commands.
...
Slash commands are powerful shortcuts that trigger specialized AI agents and workflows. They follow
the simple syntax:
...
Commands are stored in `.agents/commands/` directory and can be customized for your project needs.
...
- Produce video backgrounds and control camera movements or inpainting/outpainting workflows through
  ai-multimodal
- Process textures for WebGL, create normal/height maps, and generate sprite sheets with imagemagick
  skill pipelines
...
You can create your own commands by adding Markdown files to `.agents/commands/`:
...
Your command instructions here. Use $ARGUMENTS for all arguments. Use $1, $2, etc. for specific
arguments.
```
...
- Check if command file exists in `.agents/commands/`
...
- [Agents Code Documentation](https://docs.claude.com/en/docs/claude-code/overview)
