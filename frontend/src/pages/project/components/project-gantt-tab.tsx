...
import { TASK_PRIORITY, TASK_STATUS, TASK_TRACKER } from "@/config/entities/project.config"
...
export function ProjectGanttTab({ projectId, project }: ProjectGanttTabProps) {
  ...
  const getDueDateCol = (dueDateStr: string | null | undefined) => {
    ...
    const idx = timelineDays.findIndex(
      (day) =>
  ...
  const [newQueryFilters, setNewQueryFilters] = useState<
    Record<string, { enabled: boolean; operator: string; value: string }>
  >({})
  ...
  const renderValueSelector = (
    ...
    if (key === GANTT_FILTER_KEY.STATUS) {
      if (operator !== GANTT_FILTER_OPERATOR.LA && operator !== GANTT_FILTER_OPERATOR.KHONG_LA)
        return null
      return (
        <select
    ...
    if (key === GANTT_FILTER_KEY.TRACKER) {
      if (operator !== GANTT_FILTER_OPERATOR.IS && operator !== GANTT_FILTER_OPERATOR.IS_NOT)
        return null
      return (
        <select
    ...
    if (def.type === GANTT_FILTER_TYPE.TEXT) {
      if (operator === GANTT_FILTER_OPERATOR.NONE || operator === GANTT_FILTER_OPERATOR.ANY)
        return null
      return (
        <input
          type="text"
    ...
    if (def.type === GANTT_FILTER_TYPE.DATE) {
      if (
        operator === GANTT_FILTER_OPERATOR.ANY ||
        operator === GANTT_FILTER_OPERATOR.TODAY ||
        operator === GANTT_FILTER_OPERATOR.YESTERDAY
      )
        return null
      if (
        operator === GANTT_FILTER_OPERATOR.IN_DAYS ||
        operator === GANTT_FILTER_OPERATOR.MORE_THAN_DAYS
      ) {
        return (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Từ {format(timelineStart, "dd/MM/yyyy")} đến{" "}
                  {format(addDays(timelineStart, timelineDays.length - 1), "dd/MM/yyyy")}
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="space-y-3 flex-1">
                  {activeFilterKeys.map((key) => {
                    ...
                    const filter = Reflect.get(filterStates, key) as
                      | { enabled: boolean; operator: string; value: string }
                      | undefined
                    if (filter === undefined) return null

                    const def = Reflect.get(filterDefinitions, key) as
                      | { label: string; type: string; group: string }
                      | undefined
                      ...
                      <div key={key} className="flex flex-wrap items-center gap-2 md:gap-4 py-1">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              setFilterStates((prev) => {
                                ...
                                const existing = Reflect.get(prev, key) as
                                  | { enabled: boolean; operator: string; value: string }
                                  | undefined
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (def !== undefined) {
                              if (
                                def.type === GANTT_FILTER_TYPE.EMPLOYEE &&
                                (op === GANTT_FILTER_OPERATOR.LA ||
                                  op === GANTT_FILTER_OPERATOR.KHONG_LA) &&
                                !val
                              ) {
                                val = assignees[0]?.id || ""
                              } else if (
                                key === GANTT_FILTER_KEY.STATUS &&
                                (op === GANTT_FILTER_OPERATOR.LA ||
                                  op === GANTT_FILTER_OPERATOR.KHONG_LA) &&
                                !val
                              ) {
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            if (
                              key === GANTT_FILTER_KEY.TRACKER ||
                              key === GANTT_FILTER_KEY.PRIORITY
                            ) {
                              return (
                                <>
                            ...
                            if (def.type === GANTT_FILTER_TYPE.EMPLOYEE) {
                              return (
                                <>
                                  ...
                                  <option value={GANTT_FILTER_OPERATOR.NONE}>
                                    không phân công
                                  </option>
                                </>
                              )
                            ...
                            if (def.type === GANTT_FILTER_TYPE.TEXT) {
                              return (
                                <>
                                  ...
                                  <option value={GANTT_FILTER_OPERATOR.KHONG_CHUA}>
                                    không chứa
                                  </option>
                                  <option value={GANTT_FILTER_OPERATOR.BAT_DAU_BANG}>
                                    bắt đầu bằng
                                  </option>
                                  <option value={GANTT_FILTER_OPERATOR.KET_THUC_BANG}>
                                    kết thúc bằng
                                  </option>
                            ...
                            if (
                              def.type === GANTT_FILTER_TYPE.NUMBER ||
                              def.type === GANTT_FILTER_TYPE.PROGRESS
                            ) {
                              return (
                                <>
                                  <option value={GANTT_FILTER_OPERATOR.EQUAL}>=</option>
                                  <option value={GANTT_FILTER_OPERATOR.GREATER_THAN_EQUAL}>
                                    &gt;=
                                  </option>
                                  <option value={GANTT_FILTER_OPERATOR.LESS_THAN_EQUAL}>
                                    &lt;=
                                  </option>
                                  <option value={GANTT_FILTER_OPERATOR.NONE}>rỗng</option>
                                  <option value={GANTT_FILTER_OPERATOR.ANY}>không rỗng</option>
                  ...
                  >
                    ...
                    {Array.from(
                      ...
                      <optgroup key={group} label={group}>
                        {Object.entries(filterDefinitions)
                          .filter(
                            ([k, def]) => def.group === group && !activeFilterKeys.includes(k),
                          )
                          .map(([k, def]) => (
                            <option key={k} value={k}>
                              {def.label}
                ...
                <div className="space-y-2 pt-1">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      checked={showEstTime}
                      onChange={(e) => {
                        setShowEstTime(e.target.checked)
                      }}
                      className="rounded border-border/40 size-3.5 text-primary focus:ring-0 focus:ring-offset-0"
                    />
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      onChange={(e) => {
                        setShowAssignee(e.target.checked)
                      }}
                      className="rounded border-border/40 size-3.5 text-primary focus:ring-0 focus:ring-offset-0"
                    />
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 text-xs pt-1 border-t border-border/5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select
                value={monthInput}
                onChange={(e) => {
                  setMonthInput(parseInt(e.target.value))
                }}
              ...
              <div className="flex flex-col">
                {treeTasks.length === 0 ? (
                  ...
                  treeTasks.map((task: Task & { depth: number }) => (
                    <div
                      key={task.id}
                      className="h-16 flex items-center justify-between px-4 border-b border-border/40 hover:bg-muted/5 transition-colors"
                    >
                      <div
                      ...
                      >
                        <div className="flex items-center text-[11px] font-normal text-foreground">
                          ...
                          >
                            {task.tracker
                              ? task.tracker.charAt(0).toUpperCase() + task.tracker.slice(1)
                              : "Task"}{" "}
                            #{task.id.slice(-5)}
                          </button>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-muted-foreground mt-0.5 pl-2">
                          {task.estimatedTime && showEstTime ? (
                            <div className="flex items-center gap-1 font-normal">
                              ...
                              <span>
                                {task.spentTimes?.reduce(
                                  (s: number, st: TaskSpentTime) => s + st.hours,
                                  0,
                                ) || 0}
                                h / {task.estimatedTime}h
                              </span>
                            </div>
                          ) : null}
                        </div>
                              ...
                              <TooltipTrigger asChild>
                                <div className="flex items-center gap-1 bg-secondary border border-border/40 py-0.5 px-2 rounded-full text-[10px] font-normal cursor-default shrink-0">
                                  ...
                                  <span className="max-w-[60px] truncate">
                                    {task.assignee.fullName.split(" ").pop()}
                                  </span>
                                </div>
                              </TooltipTrigger>
                    ...
                    {timelineDays.map((day: Date, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`${dayWidth < 20 ? "text-[9px]" : "text-[11px]"} font-bold`}
                          >
                            {dayNum}
                          </span>
                        </div>
                ...
                <div className="flex flex-col relative">
                  {treeTasks.map((task: Task) => {
                    ...
                    if (task.status === "done") {
                      ...
                      containerBg =
                        "bg-indigo-950/20 dark:bg-indigo-950/60 border border-indigo-500/20"
                      progressBg = "bg-indigo-500 animate-pulse"
                      textColor = "text-indigo-900 dark:text-indigo-200"
                    } else if (task.status === "reopened") {
                      containerBg =
                        "bg-amber-950/20 dark:bg-amber-950/60 border border-amber-500/20"
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    ...
                                    >
                                      ...
                                      <span className="truncate pr-1 z-10 text-[10px] font-semibold">
                                        {task.title}
                                      </span>
                                  ...
                                  <TooltipContent className="text-xs space-y-1 rounded-xl p-3 max-w-[280px]">
                                    ...
                                    <p className="font-medium text-[10px]">
                                      ...
                                      {task.status === "done" && isCompletedEarly
                                        ? "hoàn thành sớm"
                                        : task.status}
                                      )
                                    </p>
                                    ...
                                    {isOverdue && (
                                      <p className="text-rose-500 font-bold text-[10px]">
                                        Cảnh báo: Đã quá hạn hoàn thành!
                                      </p>
                                    )}
                                  </TooltipContent>
                                </Tooltip>
        ...
        <div className="w-56 shrink-0 bg-background p-4 rounded-xl border border-border/40 shadow-sm space-y-4 relative transition-all duration-200">
          ...
          <ul className="space-y-2 text-xs">
            <li>
              <button
                onClick={() => {
                  handleQuickQuery(QUICK_QUERY_TYPE.ASSIGNED_TO_ME)
                }}
            ...
            <li>
              <button
                onClick={() => {
                  handleQuickQuery(QUICK_QUERY_TYPE.REPORTED_ISSUES)
                }}
                className="text-blue-600 dark:text-blue-400 hover:underline text-left font-medium block w-full"
              >
                Reported issues
            ...
            <li>
              <button
                onClick={() => {
                  handleQuickQuery(QUICK_QUERY_TYPE.UPDATED_ISSUES)
                }}
                className="text-blue-600 dark:text-blue-400 hover:underline text-left font-medium block w-full"
              >
                Updated issues
            ...
            <li>
              <button
                onClick={() => {
                  handleQuickQuery(QUICK_QUERY_TYPE.WATCHED_ISSUES)
                }}
          ...
          <form onSubmit={handleSubmitSaveQuery} className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label
                  htmlFor="query-name"
                  className="text-xs font-semibold text-foreground flex items-center gap-0.5"
                >
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  value={newQueryDesc}
                  onChange={(e) => {
                    setNewQueryDesc(e.target.value)
                  }}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  setNewQueryForAll(e.target.checked)
                }}
                ...
                <div className="flex items-center gap-4 text-xs font-semibold pl-1">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={
                        newQueryDefaultCols ? true : newQuerySelectedCols.includes("estimatedTime")
                      }
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={
                        newQueryDefaultCols ? true : newQuerySelectedCols.includes("assignee")
                      }
                      disabled={newQueryDefaultCols}
                      onChange={(e) => {
                        if (e.target.checked) {
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={
                        newQueryDefaultCols ? true : newQuerySelectedCols.includes("progress")
                      }
                      disabled={newQueryDefaultCols}
                      onChange={(e) => {
                        if (e.target.checked) {
            ...
            <div className="space-y-3 border-t border-border pt-3">
                    ...
                    <SelectContent className="rounded-xl bg-popover max-h-60 overflow-y-auto">
                      ...
                      {Array.from(
                        ...
                        <div key={group} className="space-y-1">
                          ...
                          {Object.entries(filterDefinitions)
                            .filter(
                              ([k, def]) => def.group === group && !newQueryFilterKeys.includes(k),
                            )
                            .map(([k, def]) => (
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {newQueryFilterKeys.map((key) => {
                  const filter = newQueryFilters[key] as
                    | { enabled: boolean; operator: string; value: string }
                    | undefined
                  if (filter === undefined) return null
                  const def = Reflect.get(filterDefinitions, key) as
                ...
                <div className="grid grid-cols-12 gap-3 items-center">
                  ...
                  <div className="col-span-5 flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setAvailSelectedKey(e.target.value)
                      }}
                    ...
                    >
                      {GANTT_AVAILABLE_COLUMNS.filter(
                        (col) => !newQuerySelectedCols.includes(col.key),
                      ).map((col) => (
                        <option
                          key={col.key}
                          value={col.key}
                  ...
                  <div className="col-span-2 flex flex-col gap-2 items-center justify-center pt-4">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (selSelectedKey) {
                          setNewQuerySelectedCols((prev) =>
                            prev.filter((k) => k !== selSelectedKey),
                          )
                  ...
                  <div className="col-span-4 flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setSelSelectedKey(e.target.value)
                      }}
                      className="w-full text-xs p-1.5 border border-border rounded-md h-24 bg-background focus:outline-none focus:ring-1 focus:ring-primary"
                    >
                  ...
                  <div className="col-span-1 flex flex-col gap-1.5 items-center justify-center pt-4">
                    <Button
                      ...
                      size="icon-xs"
                      disabled={
                        !selSelectedKey || newQuerySelectedCols.indexOf(selSelectedKey) === 0
                      }
                      onClick={() => {
                        if (selSelectedKey) {
                          setNewQuerySelectedCols((prev) => {
                    ...
                    <Button
                      ...
                      disabled={
                        !selSelectedKey ||
                        newQuerySelectedCols.indexOf(selSelectedKey) ===
                          newQuerySelectedCols.length - 1
                      }
                      onClick={() => {
                        if (selSelectedKey) {
                          setNewQuerySelectedCols((prev) => {
