...
export function CreateApplicationTimeSection({ type, form, set, employees, formIndex, onRemove }: Props) {
  ...
  // For all types except leave (which handles its own past check): date cannot be in the past
  const showDatePastError = isStartDatePast
            ...
            <div className="mb-4">
              ...
              <div className="border border-border rounded-md overflow-hidden bg-background">
                <div className="grid grid-cols-[200px_350px_1fr] bg-muted/30 border-b border-border">
                  ...
                  <div className="px-4 py-3 font-medium text-[13px] text-muted-foreground flex items-center">Thời gian</div>
                  <div className="px-4 py-3 font-medium text-[13px] text-muted-foreground flex items-center">Lý do</div>
                </div>
                <div className="grid grid-cols-[200px_350px_1fr]">
                  ...
                  <div className="p-3 border-r border-border flex flex-col gap-1 justify-center">
                    ...
                    {isStartDatePast && (
                      <span className="text-[11px] text-destructive">Thời gian không được ở quá khứ</span>
                    )}
                    {isEndDateBeforeStart && (
                      <span className="text-[11px] text-destructive">Ngày kết thúc không được nhỏ hơn</span>
                    )}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                value={form.startDate}
                onChange={(e) => { set("startDate", e.target.value); }}
                className={`w-full h-9 px-3 text-sm border rounded-md bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-primary ${showDatePastError ? 'border-destructive' : 'border-input'}`}
              />
              {showDatePastError && (
            ...
            {!(
              ...
              <div className="space-y-1.5">
                ...
                />
                {isEndDateBeforeStart && (
                  <span className="text-xs text-destructive">Ngày kết thúc không được nhỏ hơn ngày bắt đầu</span>
                )}
              </div>
            )}
            ...
            {type === APPLICATION_TYPES.OVERTIME.LABEL && (
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-foreground">
                  Số giờ tăng ca <span className="text-destructive">*</span>
                </label>
                <input
                  type="number"
                  min="0.5"
            ...
            {type === APPLICATION_TYPES.LATE_EARLY.LABEL && (
              <>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-foreground">
                    Số phút <span className="text-destructive">*</span>
                  </label>
                  <input
                    type="number"
                    min="1"
                  ...
                  >
                    ...
                    {partnerShifts.length > 0
                      ...
                      : form.swapWithDate && form.swapWithEmployeeId
                        ? <option disabled value="">Không có ca trong ngày này</option>
                        : <option disabled value="">Chọn ngày và nhân sự trước</option>
                    }
                  </select>
                </div>
                <div className="space-y-1.5">
