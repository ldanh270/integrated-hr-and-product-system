...
const VIETNAM_LUNAR_HOLIDAYS_BY_YEAR: Record<number, readonly { date: string; name: string }[]> = {
  2026: [
...
export function getVietnamHolidays(year: number): IVietnamHoliday[] {
  ...
  const lunarHolidays = (VIETNAM_LUNAR_HOLIDAYS_BY_YEAR[year] ?? []).map((holiday) => ({
    date: holiday.date,
    name: holiday.name,
    type: "national" as const,
    description: "Nhân viên được nghỉ lễ",
